...
function maxSpan(nums: number[]): number {
    if (nums.length === 0) {
        return 0; // If the array is empty, the span is 0.
    }

    let maxSpan = 1; // Initialize the maximum span to 1 (for arrays with at least one element).

    for (let i = 0; i < nums.length; i++) {
        for (let j = nums.length - 1; j >= i; j--) {
            if (nums[i] === nums[j]) {
                const span = j - i + 1; // Calculate the span.
                if (span > maxSpan) {
